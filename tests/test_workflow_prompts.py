...
class WorkflowTranslationPromptTests(unittest.TestCase):
    ...
    def test_rpgmaker_project_setup_covers_optional_text_and_widths(self):
        ...
        self.assertIn("audit the final english glossary target", lowered)
        self.assertIn("itself must carry the recognizable joke", lowered)
        self.assertIn("do not merely transliterate the name", lowered)
        ...
        investigation_flat = " ".join(investigation.split())
        self.assertIn("standalone skill reruns", investigation)
        self.assertIn("derive a short game synopsis yourself", investigation)
        self.assertIn("do not ask the user to write it", investigation)
        self.assertIn("official dlsite product page", investigation)
        self.assertIn("developer/circle", investigation)
        self.assertIn("use game-local evidence only", investigation)
        self.assertIn("not as evidence", investigation_flat)
        self.assertIn("<game>/www/data/", investigation)
        self.assertIn("<game>/ace_json/", investigation)
        self.assertIn("mapinfos.json", investigation)
        self.assertIn("events -> pages -> list", investigation)
        self.assertIn("command-level `_original`", investigation)
        self.assertIn("js/plugins.js", investigation)
        self.assertIn("applicable corpus map", investigation)
        self.assertIn("short source-derived synopsis", investigation)
        ...
        self.assertIn("already-launched workers", investigation)
        self.assertIn("perform the investigation method exactly once", investigation)
        self.assertIn("audit the final english glossary target", investigation)
        self.assertIn("itself must carry the recognizable joke", investigation)
        ...
        self.assertEqual(investigation.count("<!-- character-name-evidence -->"), 1)
        self.assertEqual(investigation.count("<!-- /character-name-evidence -->"), 1)
        self.assertIn("trustworthy creator or in-game latin spelling", investigation_flat)
        self.assertIn("demonstrated naming-family morphology", investigation_flat)
        self.assertIn("attested lexical or proper-name candidates", investigation_flat)
        self.assertIn("independent lore convergence", investigation_flat)
        self.assertIn("kana phonetics and the exact mismatch", investigation_flat)
        self.assertIn("conservative naturalized fallback", investigation_flat)
        self.assertIn("common-noun reading eligible", investigation_flat)
        self.assertIn("multiple independent naming signals converge", investigation_flat)
        self.assertIn("not an automatic winner", investigation_flat)
        self.assertNotIn("common-noun reinterpretation as ineligible", investigation_flat)
        self.assertNotIn("remove an otherwise unsupported common-noun", investigation_flat)
    ...
    def test_collected_speaker_targets_remain_provisional_until_setup(self):
        ...
        self.assertIn("longer forms, aliases, and self-identification", prompt)
        self.assertIn("explicitly curated project glossary decisions", prompt)
        self.assertIn("treat phonetic, lexical, and lore-based readings as competing", prompt)
        self.assertIn("character-name evidence hierarchy", prompt)
        self.assertIn("common-noun readings", prompt)
        self.assertIn("eligible when multiple independent naming signals support them", prompt)
        self.assertIn("multiple independent naming signals support them", prompt)
