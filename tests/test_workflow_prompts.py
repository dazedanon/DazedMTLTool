...
class WorkflowTranslationPromptTests(unittest.TestCase):
    ...
    def test_rpgmaker_project_setup_covers_optional_text_and_widths(self):
        ...
        self.assertIn("exactly three fresh subagents sequentially", lowered)
        self.assertIn("no forked conversation context", lowered)
        self.assertIn('`fork_turns="none"`', lowered)
        self.assertIn("never show a later worker any earlier report", lowered)
        ...
        self.assertIn("exactly three fresh subagents sequentially", investigation)
        self.assertIn("no forked conversation context", investigation)
        self.assertIn('`fork_turns="none"`', investigation)
        self.assertIn("never show a later worker any earlier report", investigation)
        self.assertIn("guidance files unchanged until all three", investigation)
