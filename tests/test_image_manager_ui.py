...
class ImageManagerSelectionTests(unittest.TestCase):
    ...
    def test_reopened_manager_clears_highlights_and_filters_multiple_folders(self):
        ...
        try:
            ...
            combo.set_selected_folders(set())
            combo.setFocus()
            QTest.keyClick(combo, Qt.Key_Down)
            self.app.processEvents()
            self.assertEqual(combo.selected_folders(), {extra_folders[0]})
            self.assertEqual(reopened.image_list.count(), 1)
            QTest.keyClick(combo, Qt.Key_Down)
            self.app.processEvents()
            self.assertEqual(combo.selected_folders(), {extra_folders[1]})
