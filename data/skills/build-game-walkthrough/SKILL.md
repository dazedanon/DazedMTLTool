...
description: "Build a detailed, source-traceable game walkthrough as one offline HTML application with four top-level views: Main Route, Optional Content, Bosses, and Scenes & CG. Use when Codex must audit local game events and databases, write player-usable story and optional routes without guessing from coordinates, expose expandable evidence for every entry, or extend the other walkthrough views later."
---
...
They are authoritative for player-facing names, character identities and pronouns, relationships, location spellings, fixed terminology, and project-specific English usage. Build a compact `project-context.md` in the walkthrough working directory that records every named character and place used by Main Route, Optional Content, and Bosses plus every quirk that can affect guide prose. Do not rely on memory or infer a pronoun from an actor sprite, dialogue tone, or name.
...
4. **Scenes & CG** — a polished “Coming later” placeholder.
...
Keep boss stat/strategy dossiers in Bosses and companion/relationship scene catalogs in Scenes & CG even when those systems are optional. Main Route must link directly to completed Optional Content entries at their declared first-availability anchors and to every Bosses dossier encountered on that route. Optional Content must link to the dossiers for bosses encountered inside those entries. Boss dossiers must link back to the route or optional entry that reaches them. Show a non-interactive “Detailed entry coming later” affordance only for unfinished Scenes & CG material. Never publish a dead link.
...
6. Save `systems-inventory.json` in the private walkthrough work area. For each candidate record its source/configuration signal, active-use proof, player-facing entry point if any, affected views, decision, and the focused artifacts or source IDs produced by a deep audit.
...
- Complete collectible sweeps, relationship/companion scene routes, CG catalogs, achievement audits, and 100% completion claims.
...
- `WALKTHROUGH.md` — editable Main Route, Optional Content, and Bosses source.
- `evidence.json` — source snapshots, route-claim ledger, optional dependency graph, boss dossiers, and view bindings.
...
> **AI-generated guide:** This walkthrough was created with AI by analyzing the translated game's code and data. Main progression, optional-event chains, gates, and rewards are source-traceable; directions intentionally focus on verified destinations and interactions rather than tile-perfect walking paths.
...
Treat validation errors as blockers. Publish only verified core route claims. When exact navigation cannot be proven from static data, narrow the prose instead of weakening the claim or exposing a live-play warning.
...
- Scenes & CG is a polished placeholder, not a fabricated summary.
...
Finish by reporting the published path, working-artifact path, source-backed chapter and objective-section counts, verified route-step, Optional Content entry, and boss-dossier counts, excluded-category counts, and validation status.
