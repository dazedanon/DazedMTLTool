#!/usr/bin/env python3
# -*- coding: utf-8 -*-
r"""Gakuen-derived masking primitives, adapted for Tropical Chase.

Tropical Chase uses native MZ 101 speakers, not Gakuen's leading speaker rows.
Its adapter calls mask/unmask; legacy speaker/sanitation helpers below are
historical Gakuen behavior, not rules to apply to this game. Numbered portrait
selectors and nested variables must remain indivisible. See PROVENANCE.json.
"""
...
_LEADING_CODE_RUN = re.compile(r"^(?:\\+[A-Za-z]+\[[^\]]*\]|\\+[A-Za-z.|!^<>{}$]|\s)+")
...
_CODE_RE = re.compile(
    r"""
      \\+ (?:[FM][1-8]|[A-Za-z]+) \[ (?:[^\[\]]|\[[^\[\]]*\])* \]
                                    # Tropical Chase: LL_StandingPicture F1-F8,
                                    # M1-M8, plus F3[\V[1]] nested arguments.
    | \\+ [.|!^<>{}$]                # \.  \|  \!  \^  \<  \>  \{  \}  \$
