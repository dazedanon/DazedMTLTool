...
$coldRoot = Join-Path $toolRoot 'clean_game'
...
if ($existing) { throw 'Clean game already running' }
try {
    ...
    $shot = Join-Path $toolRoot 'reports/cold_boot.png'
    & python -X utf8 (Join-Path $PSScriptRoot 'capture_window.py') $windowProcess.MainWindowHandle.ToInt64() $shot
    ...
    $result | ConvertTo-Json | Set-Content -LiteralPath (Join-Path $toolRoot 'reports/cold_boot.json') -Encoding UTF8
