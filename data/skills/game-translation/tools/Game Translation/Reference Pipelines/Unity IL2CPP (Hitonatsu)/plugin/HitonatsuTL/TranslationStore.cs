...
namespace HitonatsuTL
{
    ...
    internal static class TranslationStore
    {
        ...
        // ---- runtime harvest --------------------------------------------
        //
        // Static extraction can only find text that is stored as text. This
        // game builds its voice and expression dropdown labels at runtime by
        // slicing Addressables address strings (VoiceFaceOverrideController
        // .GetBaseSituationName, RVA 0x795200), so those labels are in no
        // asset and no metadata literal. The only way to enumerate them is to
        // watch what actually reaches a text component.
        //
        // Turn Harvest on, play through everything including Free Mode, and
        // untranslated.txt is the list of units the pipeline still owes.

        private static readonly HashSet<string> _harvested = new HashSet<string>(StringComparer.Ordinal);
        private static readonly object _harvestLock = new object();
        private static string _harvestPath;
        private const int HarvestCap = 2000;

        public static void ReportIfUntranslated(string text, string where)
        {
            if (Plugin.CfgHarvest?.Value != true) return;
            if (string.IsNullOrEmpty(text) || !HasJapanese(text)) return;
            try
            {
                lock (_harvestLock)
                {
                    if (_harvested.Count >= HarvestCap || !_harvested.Add(text)) return;
                    if (_harvestPath == null)
                    {
                        var dir = Path.GetDirectoryName(Assembly.GetExecutingAssembly().Location) ?? ".";
                        _harvestPath = Path.Combine(dir, "untranslated.txt");
                        File.WriteAllText(_harvestPath, "", Encoding.UTF8);
                    }
                    File.AppendAllText(_harvestPath,
                        where + "\t" + text.Replace("\n", "\\n").Replace("\r", "\\r") + "\n",
                        Encoding.UTF8);
                }
            }
            catch { /* harvesting must never break the game */ }
        }
