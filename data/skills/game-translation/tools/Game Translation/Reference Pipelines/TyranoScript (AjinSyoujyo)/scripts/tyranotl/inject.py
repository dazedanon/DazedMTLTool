...
def render(unit: Unit, site: Site, units: dict[str, Unit] | None = None) -> str:
    ...
    if inside_kag_attribute(site):
        # KAG's makeTag() deletes every literal space inside a quoted attribute
        # value, so English arrives as "Justtalknormally". See codes.NBSP.
        text = codes.protect_spaces(text)
    ...
    if site.form in ("attr", "nested"):
        ...
        if quote in text:
            text = text.replace(quote, "'" if quote == '"' else '"')
