...
## Session findings worth carrying to the next Unity/PlayMaker game

1. **PlayMaker `paramDataType` codes are build-specific.** `tl.py derive` solves the
   mapping and proves it by saturation (every typed column consumed exactly, zero
   violations). Never hardcode a table from another game.
2. **Classify FSM strings by SITE, not content.** Text-setter action -> display;
   StringSwitch / animator trigger -> engine key. Strings that are BOTH must be
   translated and excluded from the denylist, because the dictionary hook runs on
   the text setter, after every comparison.
3. **CSV script tables have one chokepoint**: `CsvReader.LoadFromString`. Swap the
   whole file, keyed on SHA-1 of the original, so an unrecognised CSV stays Japanese
   instead of silently receiving a stale translation.
4. **`TextAsset.text` keeps the UTF-8 BOM**, and .NET's `Trim()` does not strip
   U+FEFF. Preserve the caller's BOM state in the CSV swap; strip U+FEFF in the
   dictionary normaliser.
5. **Model JSON breaks in two specific ways** (unescaped dakuten quote `あ"`, and a
   self-corrected reply emitting two objects). Both are recoverable from stored
   batch results with no re-billing -- fix the parser and re-fetch. 4,181 -> 5,534.
6. **Line count is cosmetic; width is what clips.** The dialogue rects are 959-1025
   units inside an 800-unit canvas, so TMP's own wrap engages off-screen. Measure the
   box (`2 * 800 / fontSize` = 53 cells) and cross-check against the widest shipped
   JP line (54). See the skill's `references/text-fitting.md`.
7. **Batches can sit queued for hours.** `processing=N, succeeded=0` is queue depth,
   not progress. `tl.py live` is the synchronous fallback at 2x cost.
8. **Measured cost calibration** (Sonnet 5, 5,680 units): input estimate was accurate
   to 0.2%; output ran 1.43x the ratio-1.3 assumption (effective EN/JP 2.36 with JSON
   scaffolding). Batch+cache actual: $1.24. Live top-ups: $0.11.

## Commands

    derive extract dryrun            scope and prove the FSM decoding
    submit / status / fetch / usage  Claude batch (usage = real billed tokens)
    live                             synchronous fallback, 2x cost, minutes
    validate                         completeness / residual-JP / placeholders / layout
    reflow                           deterministic box fitting (no API)
    shorten                          re-translate what fits at no breaking
    package                          build the BepInEx payload

9. **Blank lines inside a panel are structure.** They set the block's height; losing
   one makes the block shorter and it collides with art anchored beside it. On this
   game that looked exactly like a width overflow and was not. 8 units had dropped
   one. `layout.restore_blocks` rebuilds the source's blocks and re-wraps each to its
   original line count; `fit_box` never flattens across an interior blank.
10. **The per-unit envelope needs a floor.** "This JP fitted, so this much fits" says
   nothing when the JP is 10 cells in a panel with room for sixty. Require the source
   to be multi-line AND >= ~18 cells wide before using it as a bound: 29 flagged
   units -> 13, all genuine.
11. **Re-flow after `shorten`.** Shortened text already fits, so the fitter skips it
   and the model's mid-clause breaks are what ship. Re-running the solver at the same
   line count lets the punctuation bonus fix them for free; gate on "ends more lines
   on sentence boundaries" so it can't make a hand-tuned break worse.
12. **Distribution build:** console off, disk log off, plugin QA logging off. Edit
   `BepInEx.cfg` section-aware -- several sections share the key `Enabled`, and a
   blind replace also disables the chainloader. Exclude `cache/`, logs,
   `untranslated.txt`; KEEP `winhttp.dll` + `doorstop_config.ini` +
   `.doorstop_version`. Verify by running the distribution copy and confirming the
   log files never appear.
