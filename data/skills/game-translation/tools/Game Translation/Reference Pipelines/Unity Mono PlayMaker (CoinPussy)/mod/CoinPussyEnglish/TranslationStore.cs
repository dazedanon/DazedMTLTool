...
namespace CoinPussyEnglish
{
    ...
    /// Carried over from the NTR Soccer / SheepClickerTL patches: hand-rolled
    /// flat JSON parser, format-string pattern matching for {0}-style templates,
    /// compose fallback for concatenated strings, hot-path hit/miss caches.
    ///
    /// `denylist.json` holds the strings this game uses as engine keys
    /// (StringSwitch comparisons, animator triggers, FSM names). They are never
    /// translated and never reported as missing — without it the untranslated
    /// log fills with animation names like `H_M字ピース`.
    /// </summary>
    internal static class TranslationStore
    {
        ...
        // Strings the game uses as engine keys (StringSwitch comparisons, animator
        // triggers, FSM names). Never translated, never reported. Strings that are
        // BOTH a key and a display label are excluded from this list at packaging
        // time, so they still translate.
        private static readonly HashSet<string> _deny =
        ...
        public static void LoadAll()
        {
            ...
            LoadDenylist(Path.Combine(dir, "denylist.json"));

            int total = 0, skipped = 0;
            foreach (var file in Directory.EnumerateFiles(dir, "*.json"))
            {
                var stem = Path.GetFileName(file);
                if (stem == "denylist.json" || stem == "csv_manifest.json") continue;
                try
                {
        ...
        /// <summary>Load the engine-key denylist (a flat JSON array of strings).</summary>
        private static void LoadDenylist(string path)
        {
            if (!File.Exists(path)) return;
            try
            {
                foreach (var s in JsonFlat.ParseStringArray(File.ReadAllText(path, Encoding.UTF8)))
                {
                    if (!string.IsNullOrEmpty(s)) _deny.Add(s);
                }
                Plugin.Log.LogInfo($"denylist: {_deny.Count} engine-key strings will never be translated");
            }
            catch (Exception e)
            {
                Plugin.Log.LogError($"failed to load denylist: {e.Message}");
            }
        }
        ...
        public static void ReportIfUntranslated(string s)
        {
            ...
            if (_deny.Contains(s)) return;      // an engine key, not missing text
        ...
        private static bool TryGetInternal(string original, int depth, out string translated)
        {
            ...
            if (_deny.Contains(original)) { translated = null; return false; }
            ...
            if (hasJp && Plugin.CfgFallbackContains?.Value == true)
            {
                ...
                {
                    if (_deny.Contains(kv.Key)) continue;
        ...
        // collapse literal "\n" / CRLF to \n; drop BOMs; trim whitespace.
        // U+FEFF matters: at least one hint string reaches the UI still carrying the
        // byte-order mark from the asset it was authored in, and .NET's Trim() does
        // NOT treat U+FEFF as whitespace, so without this the exact key never matches.
    ...
    internal static class JsonFlat
    {
        ...
        /// <summary>Parse a flat JSON array of strings: ["a", "b", ...].</summary>
        public static IEnumerable<string> ParseStringArray(string s)
        {
            int i = 0;
            SkipWs(s, ref i);
            Expect(s, ref i, '[');
            ...
            {
                SkipWs(s, ref i);
                yield return ReadString(s, ref i);
                SkipWs(s, ref i);
                if (i < s.Length && s[i] == ',') { i++; continue; }
                if (i < s.Length && s[i] == ']') { i++; yield break; }
                throw new FormatException($"expected ',' or ']' at offset {i}");
