...
# This game reports UE5.5 package data but ships a 5.6-era IoStore container
# header (SoftPackageReferencesOffset). retoc's UE5_5 mapping emits the older
# SoftPackageReferences header, producing a container the engine misreads
# (garbage TArray size -> FAsyncLoadingThread crash, even for unmodified assets).
# Force the matching header version while keeping UE5_5 package serialization.
if ($cfg.containerHeaderVersion) {
    $retocArgs += @("--override-container-header-version", $cfg.containerHeaderVersion)
}
