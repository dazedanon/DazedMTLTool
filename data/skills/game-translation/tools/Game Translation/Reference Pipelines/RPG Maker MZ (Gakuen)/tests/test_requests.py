...
from mztl import config, store, driver, requests as R  # noqa: E402
...
docs, glossary, reqs, id_maps, name_maps = driver.build_all(
    STORE, cfg, MODEL, 60, retranslate_all=True, effort="low", ttl=driver.BATCH_TTL)
...
check("cache breakpoint carries the batch TTL",
      all(any(b.get("cache_control", {}).get("ttl") == driver.BATCH_TTL
...
for probe in ("Azusa", "Milly", "Top Maginoir", "Lewdness",
              "Class Dragon", "Bride Thief"):
...
_d, _g, rreqs, _im, _nm = driver.build_all(
    ...
    only_ids=set(notes), effort="low", ttl=driver.BATCH_TTL,
    retry_notes=notes)
if rreqs:
    cut = 0
