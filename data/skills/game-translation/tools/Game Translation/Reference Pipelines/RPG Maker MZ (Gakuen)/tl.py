...
tl.py - the pipeline CLI.
    ...
    python tl.py plugins-src apply|check   text hardcoded in plugin CODE
...
from mztl import (config, store, extract, inject, validate, driver, estimate,
...
def cmd_inject(args):
    cfg = _cfg(args)
    # MZ puts data/ at the GAME ROOT. The MV `www/` level does not exist here,
    # and an export written under one lands a level too deep on install.
    out = args.out or os.path.join(HERE, "out", "data")
    inject.run(cfg, args.store, out_dir=None if args.in_place else out,
               in_place=args.in_place)
    if not args.in_place:
        print("\nCopy over the game's data/ (it sits at the game root on MZ), "
              "or re-run with --in-place, which backs data/ up first.")
    return 0
...
def cmd_plugins_src(args):
    """The plugin-SOURCE track: Japanese hardcoded inside plugin code.

    Separate from `plugins`, which covers plugin PARAMETERS. A plugin that
    implements a whole screen usually hardcodes most of its own UI, and that
    text is in no unit, so nothing else in this pipeline can see it."""
    cfg = _cfg(args)
    sys.path.insert(0, os.path.join(HERE, "tools"))
    import plugins_src
    root = os.path.dirname(os.path.normpath(cfg.data_dir))
    ledger = os.path.join(args.store, plugins_src.LEDGER)
    if not os.path.exists(ledger):
        print("no %s - nothing to apply" % ledger)
        return 0
    if args.action == "check":
        return plugins_src.check(root, ledger)
    return plugins_src.apply(root, ledger, jp_data=args.jp_data)
