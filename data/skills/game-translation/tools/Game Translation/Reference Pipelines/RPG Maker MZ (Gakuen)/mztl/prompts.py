...
KIND_INSTRUCTION = {
    "text": (
        ...
        "line's emotional register exactly - this game swings between "
        "school-life comedy, the heroine's anxious inner monologue and "
        "explicit non-consensual erotica, often inside one scene."
    ),
    "choice": (
    ...
    "desc": (
        "An item description drawn in a help window that is 76 cells wide and "
        "exactly TWO lines tall, so keep it short. Every number in the source "
    ...
    "ptext": (
        "A short label drawn at a FIXED screen coordinate - an ero-status row, "
        "a picture caption, a notification, or a value stored into a game "
        "variable and printed later. It has no wrapping and no box, so it must "
        "be as terse as English allows: 1-4 words. Keep a trailing colon if "
        "the source has one, because a number is drawn right after it."
    ),
    "plugin": "A plugin UI label. Terse, title case, no final punctuation.",
    "help": "One line of the choice-help window drawn above a choice list.",
}


BASE_RULES = """\
You are an expert Japanese-to-English localizer working on an adult (R18) RPG \
Maker MZ game. You translate dialogue, narration, menu and UI text, item names \
