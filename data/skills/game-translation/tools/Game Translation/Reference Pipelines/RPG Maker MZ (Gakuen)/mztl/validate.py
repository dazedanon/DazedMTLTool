...
HARD failures (block, and drive the retry queue):
    ...
    plugin-arg-unsafe-char a character the HOST command tokenises on
...
# `番` is added on this game's evidence: `五番の耳を噛む` ("bites number five's
...
def _canonical_numbers(text, currency="G"):
    ...
    # 0. native kana numerals, before anything reads the digits
...
def hard_issues(u, cfg=None, m=None):
    ...
    if u["kind"] == "ptext":
        # WHICH character is dangerous is a property of the HOST COMMAND, not
        # of the kind, and this check flagged 99 correct translations before it
        # was measured. The rule it inherited - "no ASCII space in a plugin
        # argument" - belongs to MV's code 356, whose Game_Interpreter does
        # `this._params[0].split(" ")`. **This game has zero code-356
        # commands.** Its three ptext hosts each tokenise differently:
        #
        #   355/655  CBR_EroStatus: `A.split(/\-(.*)/, 2)` - splits on the
        #            FIRST hyphen and hands everything after it to the
        #            renderer, so a space is safe and so is a later hyphen.
        #   122      the value is re-emitted inside a quoted JS string the
        #            engine eval()s, so the QUOTE character is fatal.
        #   357      the value lives in a JSON object RPG Maker parses at
        #            boot, so a double quote or a real newline is fatal.
        #
        # Injection strips each of those, so this reports what injection will
        # silently change rather than blocking.
        site = (u.get("sites") or [{}])[0]
        if "prefix" in site:
            bad = "\n" if "\n" in tl else ""
        elif u.get("quote"):
            q = u["quote"]
            bad = q if q in tl else ("\n" if "\n" in tl else "")
        elif site.get("field"):
            bad = '"' if '"' in tl else ("\n" if "\n" in tl else "")
        else:
            bad = ""
        if bad:
            out.append("plugin-arg-unsafe-char")
...
def _overflow(u, cfg, m):
    ...
    cap = cfg.hard_cap(kind, faced=bool(u.get("faced")))
    if kind in ("text", "desc"):
        f = wrap.fit(restored, width, rows, m)
        if f.overflow_rows:
            return "wraps to %d rows, box holds %d" % (f.rows, rows)
        if f.widest > cap:
            return "widest line %d cells, hard cap %d" % (f.widest, cap)
        return ""
    w = m.cells(restored)
    if w > cap:
        return "%d cells, budget %d" % (w, cap)
    return ""
...
def run(cfg, store_dir, max_show=25, verbose=True):
    ...
    if verbose:
        ...
        for k in ("identical", "residual-jp", "placeholder", "control-code",
                  ...
                  "plugin-arg-unsafe-char", "bare-escape-eats-word"):
...
RETRY_EXCLUDE = {
    "number-drift":
        "measured over the finished corpus: after canonicalising myriad "
        "grouping, English ordinals, digit+scale, bare scale words and the "
        "`+` sign, the CONFLICT bucket - where a real quantity error would "
        "have to appear - is EMPTY. All 252 remaining flags are English "
        "restructuring a quantity (`四大精霊` -> 'the four great spirits', "
        "`上位１００名` -> 'the top hundred'), and a retry produces a "
        "differently-restructured sentence, not a fixed one.",
}
