...
from mvtl import config, store, fileio, plugins_js, qa  # noqa: E402
...
# at boot).
DEV_PLUGINS = ["DRS_AllDataExtractor", "SentenceDataExtractor"]
...
def main(cfg=None, out=None, store_dir=None):
    ...
    src_data = os.path.join(os.path.dirname(HERE), "out", "www", "data")
    ...
    # --- data/*.json ------------------------------------------------------
    dest_data = os.path.join(patch, "www", "data")
    ...
    dest_js = os.path.join(patch, "www", "js")
    ...
    # --- translated images ------------------------------------------------
    img_src = os.path.join(os.path.dirname(HERE), "out", "img")
    img_n = 0
    if os.path.isdir(img_src):
        for root, _dirs, files in os.walk(img_src):
            for fn in files:
                ...
                dst = os.path.join(patch, "www", "img", rel)
