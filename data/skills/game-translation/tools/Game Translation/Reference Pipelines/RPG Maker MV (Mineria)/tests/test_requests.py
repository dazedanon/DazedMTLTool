...
# -*- coding: utf-8 -*-
"""
test_requests.py - prove the request set is well formed before a run pays for it.
...
from mvtl import config, store, driver, requests as R  # noqa: E402
...
docs, glossary, reqs, id_maps, name_maps = driver.build_all(
    STORE, cfg, MODEL, 60, retranslate_all=True, effort="low", ttl="1h")
...
check("cached prefix is over the ~1024-token minimum", ptok > 4096,
      ...
      "error)" % ptok)
check("cache breakpoint carries the 1h TTL for batch",
      all(any(b.get("cache_control", {}).get("ttl") == "1h"
...
for uid in pending_ids:
    if uid in deduped_reps:
        continue
    u = by_id[uid]
    if u["kind"] not in store.DEDUPED_KINDS:
        missing.append(uid)
        continue
    # a deduped unit is legitimately absent when a sibling represents it
    key = (u["kind"], u["src"])
    if not any(by_id[r]["kind"] == u["kind"] and by_id[r]["src"] == u["src"]
               for r in deduped_reps):
        missing.append(uid)
...
for probe in ("Mineria", "Samera", "Demon Lord", "Lewdness", "TWO VOICES"):
    check("roster/bible carries %r" % probe, probe in sysall)
...
_d, _g, rreqs, _im, _nm = driver.build_all(
    ...
    only_ids=set(notes), effort="low", ttl="1h", retry_notes=notes)
if rreqs:
    cut = 0
