...
report=bc.read(release/'narration-layout-report.json')
...
manifest['runtime_tests'].append('Narration-corrected drop-in project payload launched to the title menu with the credited caption; player save unchanged')
...
files=[ROOT/name for name in ('README.md','narration_audit.py','layout_audit.py',
...
files += [p for p in HERE.rglob('*') if p.is_file()
          and not set(p.relative_to(HERE).parts) & {'drop-in-test','build','build-final','canary','final-decompiled'}
          and p.name!='hall-1920.png'
...
bc.write(stable/'followup-narration/delivery-report.json',receipt)
