...
# The shipped lang.js substitutes { name }, { line }, etc. Keep these, and
# JavaScript template expressions, opaque before the tag matcher sees [index].
FORMAT_RE = re.compile(r"\$\{[^{}]*\}|\{\s*[A-Za-z_][\w.]*\s*\}|\{\{[^{}]*\}\}")
MASK_RE = re.compile(f"{FORMAT_RE.pattern}|{TAG_RE.pattern}|{HTML_RE.pattern}")
