...
def build_all(store_dir, cfg, model, max_units, retranslate_all=False,
              ...
              effort="low", ttl="5m", retry_notes=None, roster_min=0,
              dedup_dialogue=True):
    ...
    if include_text:
        for ch in R.build_chunks(docs, max_units, retranslate_all, only_ids,
                                 context_lines=cfg.get("context_lines", 3),
                                 dedup_dialogue=dedup_dialogue):
            note = ""
            if retry_notes:
                seen = []
                for u in ch["units"]:
                ...
                if seen:
                    note = ("Fix these named failure modes: " + "; ".join(seen)
                            ...
                            "the source has.")
                    # A width failure needs a DIFFERENT instruction from a
                    # correctness one, and it needs the budget in the note.
                    # Telling the model to "keep UI labels tight" up front does
                    # not bound them; re-requesting the overflowing ones with
                    # "now 282px, needs <= 264px, about 6% shorter" does.
                    if any("px vs box" in s for s in seen):
                        note += (" SHORTEN the ones marked with a pixel width: "
                                 "each says what it measures now and what its "
                                 "box holds. Cut words, not meaning - drop an "
                                 "article, use the shorter synonym, lose the "
                                 "'the'. These are one-line labels that are CUT "
                                 "at the box edge, so a shorter correct label "
                                 "beats a longer one that is invisible.")
            req, idmap = R.build_request(
                ch, model, store_dir, glossary, effort=effort, ttl=ttl,
                retry_note=note, roster_min=roster_min,
                roster_field_chars=cfg.get("roster_field_chars",
                                           R.prompts.ROSTER_FIELD_CHARS))
            reqs.append(req)
...
def _parse_message(message):
    # `parse.parse` does NOT fold curly quotes - `parse.normalize_reply` does,
    # and it has to be called explicitly. The adaptation dropped this call and
    # left a comment claiming otherwise, which made a whole recovery path dead:
    # a reply that uses curly quotes as its string delimiters parses in the
    # reference pipelines and did not here. Results live on the server for 29
    # days, so that was lost WORK rather than lost money - but it was lost.
...
def submit(store_dir, cfg, model, max_units, retranslate_all=False,
           ...
           warm=True, ttl=None, dedup_dialogue=True):
    ttl = ttl or cfg.get("batch_ttl", "5m")
    docs, glossary, reqs, id_maps, name_maps = build_all(
        store_dir, cfg, model, max_units, retranslate_all, include_names,
        include_text, only_ids, effort, ttl=ttl, retry_notes=retry_notes,
        roster_min=roster_min, dedup_dialogue=dedup_dialogue)
    if not reqs:
        return None, 0
    cl = C.get_client()
    ...
    store.save_state(store_dir, {
        ...
        "phase": label, "n_requests": len(reqs), "ttl": ttl,
        "dedup_dialogue": dedup_dialogue,
...
def live(store_dir, cfg, model, max_units, retranslate_all=False,
         ...
         roster_min=0, dedup_dialogue=True):
    ...
    if include_names:
        ...
        if nreqs:
            ...
            if verbose:
                ...
                # Print them HERE. The names-only path returns before the
                # shared error report at the end of this function, so a failed
                # name request used to surface as a bare "ERROR" on the
                # progress line with the reason discarded - and the reason was
                # a ValueError raised before the request was even sent.
                for cid, why in (errs + [("apply", e) for e in aerrs])[:20]:
                    print("  ! %s: %s" % (cid, why))
    ...
    docs, glossary, reqs, id_maps, _nm = build_all(
        ...
        effort=effort, ttl=cfg.get("live_ttl", "5m"),
        retry_notes=retry_notes, roster_min=roster_min,
        dedup_dialogue=dedup_dialogue)
    if not reqs:
        if verbose:
...
def _apply_and_save(store_dir, docs, glossary, results, id_maps,
    ...
    harmlessly."""
    R.apply_results(docs, glossary, results, id_maps, {}, dedup_dialogue)
    store.save_docs(docs)
    store.save_glossary(store_dir, glossary)
...
def smoke(store_dir, cfg, model, max_units, effort="low", verbose=True):
    ...
    docs, glossary, reqs, id_maps, name_maps = build_all(
        ...
        ttl=cfg.get("batch_ttl", "5m"), roster_min=cfg.get("roster_min", 0),
        dedup_dialogue=cfg.get("dedup_dialogue", True))
