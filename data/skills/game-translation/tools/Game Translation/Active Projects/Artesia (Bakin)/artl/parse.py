...
1. **An unescaped `"` inside a value.** The Japanese uses an ASCII double quote
   as a dakuten on slurred moans (`あ"っ`), and the model carries it into
...
2. **Self-correction.** The reply emits a partial object, reconsiders in prose,
   ...
   A first-`{`-to-last-`}` span swallows the prose. Extract every BALANCED
...
string boundaries. On a real run this recovered 4,181 -> 5,534 applied units.
"""
...
def _repair(s):
    ...
    # The lookahead is what stops this rule destroying a legitimate empty
    # string `:""`.
...
def _escape_inner_quotes(s):
    """Escape a `"` that cannot be a structural delimiter.

    Walks the text tracking whether we are inside a string. A `"` inside a
    string closes it only when the next non-space char is `,` `}` `]` or `:`;
    otherwise it is literal and gets a backslash."""
    out = []
    i = 0
    n = len(s)
    in_str = False
...
def _regex_fallback(s):
    """Last resort for a blob that never parses.

    Turns a truncated response into a PARTIAL-count failure the length check
    catches, instead of a total loss."""
    out = {}
...
def normalize_reply(text):
    """Symmetric to codes.clean_source's quote fold: run the same table over
    the reply so a smart quote the model echoed does not break json.loads."""
