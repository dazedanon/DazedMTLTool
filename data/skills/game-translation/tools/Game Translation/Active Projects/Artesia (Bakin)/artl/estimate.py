...
def run(cfg, store_dir, model, max_units, retranslate_all=False,
    ...
    # Pending SOURCE tokens, counted over the DEDUPED set - the model is only
    # sent one representative per dedup key, so counting every unit would
    # overstate this game's dialogue by 3.7x.
    seen = set()
    for _doc, u in store.all_units(docs):
        if not (retranslate_all or not (u.get("tl") or "").strip()):
            continue
        k = store.dedup_key(u, cfg.get("dedup_dialogue", True))
        if k is not None:
            if k in seen:
                continue
            seen.add(k)
    ...
    if verbose:
        ...
        print("  distinct cached prefixes: %d  (%d tok total, re-read %d times)"
              % (len(prefix_count), cache_write, len(reqs) - len(prefix_count)))
        print("     !! the tiktoken proxy UNDERSTATES an English prefix "
              "against Anthropic's tokenizer - measured on this game, 22,119 "
              "proxied against 32,221 actual, x1.46. The JP payload is "
              "accurate; the PREFIX is not, and on a batch the prefix is most "
              "of the bill. Use `tl.py exact` or a cold `smoke`'s "
              "cache_creation_input_tokens for the real figure.")
        for pfx, n in sorted(prefix_count.items(), key=lambda kv: -kv[1]):
            note = ""
            if prefix_tok[pfx] < 1024:
                note = ("   !! under ~1024 tokens: this silently does NOT "
                        "cache and returns no error")
            print("     %6d tok x %4d request(s)%s" % (prefix_tok[pfx], n, note))
        # Two is correct and expected: the names pass and the text pass are
        # different jobs with different system prompts. A THIRD means the block
        # is not byte-stable within a job, and every extra one pays a full
        # cache write that is never read back.
        if len(prefix_count) > 2:
            print("     !! more than 2 distinct prefixes means a cached block "
                  "is not byte-stable WITHIN a job; every extra one pays a "
                  "full cache write and is never re-read.")
...
def exact(cfg, store_dir, model, max_units, effort="low", limit=3):
    ...
    _docs, _g, reqs, id_maps, _nm = driver.build_all(
        store_dir, cfg, model, max_units, effort=effort,
        ttl=cfg.get("batch_ttl", "5m"), roster_min=cfg.get("roster_min", 0),
        dedup_dialogue=cfg.get("dedup_dialogue", True))
    cl = C.get_client()
    total = 0
