...
class RbPack:
    def __init__(self, path):
        ...
        # Older launchers (Bakin r64268) write the splash label straight after
        # the discarded int64; newer ones (r73294) precede it with a hasLabel
        # bool. Both parses are tried and the DRM MD5 decides which was right,
        # so the header layout is measured per game rather than assumed.
        for self.has_label_flag in (False, True):
            try:
                self._parse_tail(head, 16)
            except (IndexError, UnicodeDecodeError, struct.error):
                continue
            if self.check_drm() is not None:
                break
        else:
            raise ValueError("rbpack header did not parse under either layout")

    def _parse_tail(self, head, p):
        if self.has_label_flag:
            flag = head[p]
            p += 1
        else:
            flag = 1
