...
The two-line logo is replaced (set as **three** lines in English, because 44
characters against 16 cannot share a size on a 1020px canvas). The circle
badge `さざめき通り` in the bottom-left corner is **kept in Japanese on
purpose**: it is the developer's circle name - a brand mark, not game text -
per the user's call. The measured badge geometry stays in
`images/probe_circle.py` should that ever change.
