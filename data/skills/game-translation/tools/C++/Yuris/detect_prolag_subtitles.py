...
OUT_JSON = Path("work/prolag_detected_segments.json")
OUT_SHEET = Path("work/prolag_detected_segments.png")
...
def subtitle_core_mask(frame: np.ndarray) -> np.ndarray:
    ...
    for i in range(1, num):
        ...
        if w < 2 or w > 70:
            continue
        # Keep only the subtitle rows; avoid border ornaments and large bright scene art.
        if y < 18 or y + hgt > 98:
            continue
...
def changed(prev: np.ndarray, cur: np.ndarray) -> float:
    if prev is None:
        return 1.0
...
def main() -> None:
    ...
    for frame_idx in range(frame_count):
        ...
        if active is not None and has_text:
    ...
    # Merge very short flicker fragments into their predecessor.
    merged: list[Segment] = []
    for seg in segments:
    ...
    for idx, seg in enumerate(merged, 1):
        ...
        # End times are subtitle-exclusive in ASS/SRT.
