...
SINGLE_CHAR_LABELS = set("左右上下前後奥手指脚足口胸尻")
...
# are a relative skip whose anchor is *after* the RValuePointer (so iCode body
# starts at icode_operand + 4 + 8). We do not relocate it automatically because
# the patcher would have to know the exact runtime size of the RValuePointer
# slot. Instead we treat the body range as a "do not grow inside" zone and
# fail closed if a translation would change its size.
KISMET_UNRELOCATED_SKIP_KINDS = {"context_skip"}
KISMET_LITERAL_KINDS = {"skip_offset_const"}
KISMET_RELOCATED_TARGET_KINDS = KISMET_ABSOLUTE_TARGET_KINDS | KISMET_RELATIVE_SKIP_KINDS
...
def adjust_kismet_flow_targets(old_blob, new_blob, changes):
    ...
    for candidate in select_script_bytecode_candidates(old_blob, changes):
        ...
        # Fail-closed safeguard for unhandled relative skips (currently only
        # context_skip): if any string growth lands inside the body that the
        # operand was sized to skip past, the operand value would silently be
        # wrong and the runtime would jump into the middle of an instruction.
        # Refuse the rewrite so the caller skips the asset rather than guesses.
        for operand in walker.jump_operands:
            if operand["kind"] != "context_skip":
                continue
            body_start = operand["icode_operand"] + 4 + KISMET_CONTEXT_RVALUE_ICODE_SIZE
            body_end = body_start + operand["target"]
            for old_icode_end, delta in icode_change_bounds:
                if delta and body_start < old_icode_end <= body_end:
                    raise RuntimeError(
                        "context_skip body would change size after edit; refusing to "
                        f"relocate Kismet flow targets (operand iCode {operand['icode_operand']}, "
                        f"body iCode [{body_start}, {body_end}))"
                    )

        for operand in walker.jump_operands:
            kind = operand["kind"]
            if kind in KISMET_LITERAL_KINDS or kind in KISMET_UNRELOCATED_SKIP_KINDS:
                continue
        ...
        for pointer, raw_rows in raw_rows_by_pointer.items():
            ...
            for group_rows in sorted(raw_groups.values(), key=lambda rs: int(rs[0]["raw_offset"]), reverse=True):
                ...
                if row["encoding"] == "utf16le-null":
                    ...
                    replacement = encode_unreal_string_fixed_span(
                        translation,
                        row["encoding"],
                        current["end"] - offset,
                    )
                    if replacement is None:
                        print(
                            f"skipped fixed-span raw string in {rel} at {pointer}+{offset}: "
                            "translation does not fit without moving non-bytecode data"
                        )
                        continue
                else:
