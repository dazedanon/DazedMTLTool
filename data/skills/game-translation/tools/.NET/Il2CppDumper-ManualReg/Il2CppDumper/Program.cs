...
namespace Il2CppDumper
{
    class Program
    {
        ...
        private static bool Init(string il2cppPath, string metadataPath, out Metadata metadata, out Il2Cpp il2Cpp)
        {
            ...
            try
            {
                if (config.ForceManualRegistration)
                {
                    // FORK: bypass auto-search entirely. Treat the input as a flat in-memory
                    // dump (file offset == RVA) and use the manually-supplied registration VAs.
                    var imageBase = Convert.ToUInt64(config.ForceDumpImageBase, 16);
                    var codeRegistration = Convert.ToUInt64(config.ForceCodeRegistration, 16);
                    var metadataRegistration = Convert.ToUInt64(config.ForceMetadataRegistration, 16);
                    Console.WriteLine($"Manual registration mode: imageBase=0x{imageBase:X} " +
                                      $"CodeRegistration=0x{codeRegistration:X} MetadataRegistration=0x{metadataRegistration:X}");
                    if (il2Cpp is PE peDump && imageBase != 0)
                    {
                        peDump.LoadFromMemory(imageBase); // sets ImageBase and makes sections flat (RawData=VirtualAddress)
                        il2Cpp.IsDumped = true; // only flat in-memory dumps are "dumped"; on-disk PE keeps IsDumped=false
                    }
                    il2Cpp.Init(codeRegistration, metadataRegistration);
                    if (il2Cpp.Version >= 27 && il2Cpp.IsDumped)
                    {
                        var typeDefM = metadata.typeDefs[0];
                        var il2CppTypeM = il2Cpp.types[typeDefM.byvalTypeIndex];
                        metadata.ImageBase = il2CppTypeM.data.typeHandle - (il2Cpp.Version < 38 ? metadata.header.typeDefinitionsOffset : metadata.header.typeDefinitions.offset);
                    }
                    return true;
                }

                Console.WriteLine("Searching...");
                // FORK: if the input is a flat in-memory dump (il2dump/x64dbg), flatten it and search the
                // whole image so the metadata-anchored search works on protected binaries with no manual
                // CodeRegistration/MetadataRegistration input.
                if (il2Cpp is PE peAuto && peAuto.EnableDumpModeIfMemoryDump(il2cppBytes.Length))
                {
                    Console.WriteLine($"Detected a flat memory dump (imageBase=0x{il2Cpp.ImageBase:X}); flattened sections, searching the whole image.");
                }
