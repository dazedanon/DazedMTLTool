...
namespace Il2CppDumper
{
    public abstract class Il2Cpp : BinaryStream
    {
        ...
        // FORK: cheap structural validation of a parsed Il2CppCodeRegistration — used to auto-detect the
        // legacy 15-field layout. Resolves the first codeGenModule and checks its name reads as a *.dll.
        private bool LooksLikeValidCodeRegistration(Il2CppCodeRegistration cr, ulong limit)
        {
            try
            {
                if (cr.codeGenModulesCount == 0 || cr.codeGenModulesCount > 0x10000)
                    return false;
                if (cr.invokerPointersCount > limit)
                    return false;
                var mods = MapVATR<ulong>(cr.codeGenModules, cr.codeGenModulesCount);
                var first = MapVATR<Il2CppCodeGenModule>(mods[0]);
                var name = ReadStringToNull(MapVATR(first.moduleName));
                return name.Length > 0 && name.EndsWith(".dll", StringComparison.Ordinal);
            }
            catch
            {
                return false;
        ...
        public virtual void Init(ulong codeRegistration, ulong metadataRegistration)
        {
            ...
            // FORK: some binaries report metadata v29.1+ (e.g. certain v31 builds such as Blue Archive)
            // but ship the older 15-field Il2CppCodeRegistration WITHOUT the unresolvedInstance/Static
            // split. The 17-field parse then misreads codeGenModules (garbage / crash). Detect that by
            // validating the codeGenModules chain; if it doesn't resolve but the 15-field layout does,
            // use it. No Version change => metadata parsing and normal v29.1+ binaries are untouched
            // (their chain validates, so this never fires).
            if (Version >= 29.1 && !LooksLikeValidCodeRegistration(pCodeRegistration, limit))
            {
                var legacy = MapVATR<Il2CppCodeRegistrationV27NoUnresolvedSplit>(codeRegistration).ToFull();
                if (LooksLikeValidCodeRegistration(legacy, limit))
                {
                    Console.WriteLine("Detected legacy 15-field Il2CppCodeRegistration (no unresolved instance/static split); re-reading.");
                    pCodeRegistration = legacy;
                }
            }
            ...
            for (var i = 0; i < pMetadataRegistration.typesCount; ++i)
            {
                ...
                typeDic[pTypes[i]] = types[i]; // FORK: tolerate duplicate/null type pointers in dumped images
            }
            if (Version >= 24.2)
            {
                var pCodeGenModules = MapVATR<ulong>(pCodeRegistration.codeGenModules, pCodeRegistration.codeGenModulesCount);
            ...
            // FORK: the generic-method-table block is not needed for signatures/RVAs and is the
            // most fragile when a dumped image's MetadataRegistration tail is imperfect. Guard it.
            try
            {
                ...
                {
                    if (table.genericMethodIndex < 0 || table.genericMethodIndex >= methodSpecs.Length) continue;
                    ...
                    list.Add(methodSpec);
                    if (table.indices.methodIndex >= 0 && table.indices.methodIndex < genericMethodPointers.Length)
                        methodSpecGenericMethodPointers[methodSpec] = genericMethodPointers[table.indices.methodIndex];
                }
            }
            catch (Exception ex)
            {
                Console.WriteLine($"WARN: generic method table skipped ({ex.GetType().Name}) - signatures/RVAs unaffected");
            }
        }
