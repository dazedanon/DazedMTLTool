...
namespace Il2CppDumper
{
    public sealed class PE : Il2Cpp
    {
        ...
        // FORK: auto-detect a flat in-memory dump (file offset == RVA, e.g. produced by il2dump/x64dbg).
        // Such a file spans the whole VIRTUAL image and is larger than the on-disk RAW extent, unlike a
        // normal on-disk PE. If detected, flatten the sections so MapVATR works on the dump, and switch
        // to whole-image search (the protector may have stripped/renamed section characteristics, so the
        // Characteristics-based section filter would otherwise miss the registration data).
        public bool EnableDumpModeIfMemoryDump(long fileLength)
        {
            ulong virtEnd = 0, rawEnd = 0;
            foreach (var s in sections)
            {
                virtEnd = Math.Max(virtEnd, (ulong)s.VirtualAddress + s.VirtualSize);
                rawEnd = Math.Max(rawEnd, (ulong)s.PointerToRawData + s.SizeOfRawData);
            }
            if ((ulong)fileLength >= virtEnd && (ulong)fileLength > rawEnd)
            {
                LoadFromMemory(ImageBase); // ImageBase came from the dump's PE header (preferred base)
                IsDumped = true;
                dumpMode = true;
                return true;
            }
        ...
        public override SectionHelper GetSectionHelper(int methodCount, int typeDefinitionsCount, int imageCount)
        {
            if (dumpMode)
            {
                // search the entire flat image (exec/data/bss all cover [base, base+len)); the anchor
                // checks (typeDefinitionsCount pair, codeGenModules count == imageCount) stay specific.
                var whole = new SearchSection
                {
                    offset = 0,
                    offsetEnd = Length,
                    address = ImageBase,
                    addressEnd = ImageBase + Length,
                };
                var sh = new SectionHelper(this, methodCount, typeDefinitionsCount, metadataUsagesCount, imageCount);
                sh.SetSection(SearchSectionType.Exec, whole);
                sh.SetSection(SearchSectionType.Data, whole);
                sh.SetSection(SearchSectionType.Bss, whole);
                return sh;
            }
